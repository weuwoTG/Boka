...
if ! [ -x "$(command -v docker-compose)" ]; then
    ...
    if [ -f /etc/debian_version ]; then
        sudo apt-get install \
            ...
            curl \
            gnupg-agent \
            software-properties-common -y 1>boka-install.log 2>&1
        curl -fsSL https://download.docker.com/linux/ubuntu/gpg |
            sudo apt-key add - 1>boka-install.log 2>&1
        sudo add-apt-repository \
            "deb [arch=amd64] https://download.docker.com/linux/ubuntu \
            $(lsb_release -cs) \
            stable" 1>boka-install.log 2>&1
        sudo apt-get update -y 1>boka-install.log 2>&1
        sudo apt-get install docker-ce docker-ce-cli containerd.io -y 1>boka-install.log 2>&1
    elif [ -f /etc/arch-release ]; then
        sudo pacman -Syu docker --noconfirm 1>boka-install.log 2>&1
    elif [ -f /etc/redhat-release ]; then
        sudo yum install -y yum-utils 1>boka-install.log 2>&1
...
sudo docker-compose build 1>boka-install.log 2>&1
printf "\033[0;32m - success\e[0m\n"

printf "\033[0;32mStarting Boka setup in console mode...\e[0m\n"
