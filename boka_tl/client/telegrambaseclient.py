...
class TelegramBaseClient(abc.ABC):
    ...
    def __init__(
        ...
        *,
        ...
        self._appconfig_handle = None
        self._last_request = time.time()
        self._no_updates = not receive_updates
        self.appconfig = AppConfig(self)
    ...
    async def connect(self: "TelegramClient") -> None:
        ...
        if self._catch_up:
            ...
            for (
                entity_id,
                state,
            ) in self.session.get_update_states():                    
                if entity_id == 0:
            ...
            for state in cs:
                try:
                    entity = self.session.get_input_entity(
                        state.channel_id
                    )                    
                except ValueError:
        ...
        self._appconfig_handle = self.loop.create_task(self.appconfig._refresh_loop())
    ...
    def _save_states_and_entities(self: "TelegramClient"):                    
        ...
        self.session.set_update_state(
            0, types.updates.State(**ss, unread_count=0)
        )                    
    ...
    async def _disconnect(self: "TelegramClient"):
        ...
        await helpers._cancel(
            ...
            appconfig_handle=self._appconfig_handle,
        )
