...
- **Песочница модулей** (`boka/secure_local.py`) — сторонние модули работают с заблокированными импортами (`os`, `subprocess`, `socket`, `requests`, `aiohttp`, `pyrogram`, …) и «оглушёнными» опасными встроенными функциями (`open`, `exec`, `eval`, `compile`).
