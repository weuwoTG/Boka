...
class Database(dict):
    ...
    async def ensure_content_channel(self):
        ...
        existing_channel_id = self.get("boka.forums", "channel_id", None)
        ...
        if existing_channel_id:
            try:
                ...
                content_channel = None
                self.set("boka.forums", "forums_cache", {"boka-userbot": {}})
        ...
        if not content_channel:
            async for dialog in self._client.iter_dialogs():
                if dialog.title and "boka-userbot" in dialog.title.lower():
                    ...
                    self.set("boka.forums", "channel_id", int(dialog.entity.id))
                    break
        ...
        if not content_channel:
            ...
            self.set("boka.forums", "channel_id", int(content_channel.id))
    ...
    def read(self):
        ...
        try:
            ...
            if re.search(r'"(hikka\.)(\S+\":)', db):
                ...
                db = re.sub(r"(hikka\.)(\S+\":)", lambda m: "boka." + m.group(2), db)
            if re.search(r'"(legacy\.)(\S+\":)', db):
                logging.warning("Converting db after update")
                db = re.sub(r"(legacy\.)(\S+\":)", lambda m: "boka." + m.group(2), db)
    ...
    async def store_asset(self, message: Message) -> int:
        ...
        try:
            _assets_topic_id = self.get("boka.forums", "forums_cache", {})[
                "boka-userbot"
            ]["Assets"]
        except (TypeError, KeyError):
            raise NoAssetsChannel("Tried to save asset to non-existing asset topic.")

        if not (_content_channel_id := self.get("boka.forums", "channel_id", None)):
