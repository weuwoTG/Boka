...
from boka_tl.errors import FloodWaitError, RPCError
...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.boka_me.id
        self._cached_usernames = set()

        if self._client.boka_me.username:
            self._cached_usernames.add(self._client.boka_me.username.lower())

        if self._client.boka_me.usernames:
            self._cached_usernames.update(
                u.username.lower()
                for u in getattr(self._client.boka_me, "usernames", [])
            )

        self._cached_usernames.add(str(self._client.boka_me.id))
