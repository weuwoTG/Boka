...
from boka_tl.tl.functions.account import UpdateNotifySettingsRequest
...
class Module:
    ...
    @property
    def boka_commands(self) -> dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def boka_inline_handlers(self) -> dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def boka_callback_handlers(self) -> dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def boka_watchers(self) -> dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @boka_commands.setter
    def boka_commands(self, _):
        pass
    ...
    @boka_inline_handlers.setter
    def boka_inline_handlers(self, _):
        pass
    ...
    @boka_callback_handlers.setter
    def boka_callback_handlers(self, _):
        pass
    ...
    @boka_watchers.setter
    def boka_watchers(self, _):
        pass
    ...
    async def _decline(
        ...
        self._db.set(
            "boka.main",
            "declined_joins",
            list(set(self._db.get("boka.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def request_join(
        ...
        match channel:
            ...
            case _ if channel.id in self._db.get("boka.main", "declined_joins", []):
                if assure_joined:
                    raise LoadError(
        ...
        self.boka_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "boka_wait_channel_approve")
        ...
        *,
        ...
        if re.search(r"# ?scope: ?boka_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?boka_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Boka version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"boka.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("boka.main", "stats", True)
            and url is not None
