...
from boka_tl import events
from boka_tl.errors import (
    ApiIdInvalidError,
...
from boka_tl.errors.rpcerrorlist import (
    AuthKeyUnregisteredError,
    YouBlockedUserError,
)
from boka_tl.network.connection import (
    ConnectionTcpFull,
    ConnectionTcpMTProxyRandomizedIntermediate,
)
from boka_tl.password import compute_check
from boka_tl.sessions import MemorySession, SQLiteSession
from boka_tl.tl.functions.account import GetPasswordRequest
from boka_tl.tl.functions.auth import CheckPasswordRequest
from boka_tl.tl.functions.contacts import UnblockRequest
...
class Boka:
    ...
    def _migrate_sessions(self):
        ...
        with os.scandir(BASE_DIR) as entries:
            legacy = [
                ...
                if entry.is_file()
                and entry.name.startswith("boka-")
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.boka_db = database.Database(client)
        await client.boka_db.init()

        try:
            db = client.boka_db
            existing = db.get("boka.inline", "custom_bot", False)
        except Exception:
            existing = False
        ...
        if (
            ...
            while bot := input(
                "You can enter a custom bot username or leave it empty and Boka will generate a random one: "
            ):
                bot = bot.strip()
                bot = bot.lstrip("@")
                if any(
                ...
                try:
                    if await self._check_bot(client, bot):
                        db.set("boka.inline", "custom_bot", bot)
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
                ...
                await client.boka_inline.bot.send_photo(
                    ...
                    caption=(
                        ...
                        (
                            ...
                            if client.boka_me.premium is True
                            else "🪐 Boka"
                        ),
                        ".".join(list(map(str, list(__version__)))),
                        build,
            ...
            logging.debug(
                ...
                client.tg_id,
                client.boka_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
