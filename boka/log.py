...
import boka_tl
from boka_tl.errors import PersistentTimestampOutdatedError, TimeoutError
from boka_tl.errors.rpcbaseerrors import ServerError, RPCError
from boka_tl.errors.rpcerrorlist import FloodWaitError
...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of Boka modules properly.
    """

    try:
        if filename.startswith("<") and filename.endswith(">"):
            module = filename[1:-1].split(maxsplit=1)[-1]
            if (module.startswith("boka.modules")) and module in sys.modules:
                return list(
                    map(
                        lambda x: f"{x}\n",
        ...
        def to_hashable(dictionary: dict) -> dict:
            ...
            for key, value in dictionary.items():
                match value:
                    ...
                    case boka_tl.TelegramClient() | CustomTelegramClient():
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.boka_caller or client_id == record.boka_caller)
        ]
    ...
    async def _show_full_trace(
        ...
        chunks = list(utils.smart_split(*boka_tl.extensions.html.parse(chunks), 4096))
    ...
    async def sender(self):
        async with self._send_lock:
            ...
            for client_id in self._mods:
                ...
                funcs = []
                for item in self.tg_buff:
                    if not isinstance(item[0], BokaException):
                        continue
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_boka_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_boka_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        if record.levelno >= self.tg_level:
            if record.exc_info:
                ...
                exc = BokaException.from_exc_info(
                    *record.exc_info,
...
async def check_branch(me_id: int, allowed_ids: list, self):
    if os.environ.get("BOKA_NO_GIT") == "1":
        return
...
def init():
    ...
    logging.getLogger("boka_tl.network").addFilter(NoFetchUpdatesFilter())
    ...
    logging.getLogger("boka_tl").setLevel(logging.WARNING)
