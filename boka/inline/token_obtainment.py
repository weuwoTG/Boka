...
from boka_tl.errors.rpcerrorlist import YouBlockedUserError
...
class TokenObtainment(InlineUnit):
    async def _create_bot(self: "InlineManager"):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("boka.inline", "custom_bot", False):
                username = self._db.get("boka.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
            ...
            try:
                ...
                if "DOCKER" in os.environ:
                    m = await conv.send_file(
                        "https://raw.githubusercontent.com/weuwoTG/Boka/refs/heads/master/assets/boka-ava.png"
                    )
                else:
                    m = await conv.send_file(
                        main.BASE_PATH / "assets" / "boka-ava.png"
                    )
                r = await conv.get_response()
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    ...
                    if self._db.get("boka.inline", "custom_bot", False) and (
                        self._db.get("boka.inline", "custom_bot", False) != btn_text
                    ):
                        continue

                    if not self._db.get("boka.inline", "custom_bot", False) and not (
                    ...
                    self._db.set("boka.inline", "bot_token", token)
                    self._token = token
