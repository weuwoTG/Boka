...
from boka_tl import TelegramClient, events
...
from boka_tl.sessions import SQLiteSession
from boka_tl.tl.functions.contacts import UnblockRequest
from boka_tl.tl.functions.messages import (
...
class InlineManager(
    ...
    :type client: boka.tl_cache.CustomTelegramClient
    :type db: boka.database.Database
    :type allmodules: boka.loader.Modules
    """
    ...
    def __init__(
        ...
        self._token = db.get("boka.inline", "bot_token", False)
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.boka_me)
        ...
        self._bot_client = TelegramClient(
            SQLiteSession(
                os.path.join(main.SESSIONS_DIR, f"boka-{self._me}-bot-{bot_uid}")
            ),
            self._client.api_id,
        ...
        try:
            ...
            self._bot_client.boka_me = bot_me
        ...
        for folder in _folders.filters:
            if getattr(folder, "title", None) == "Boka":
                if any(
                    [
    ...
    async def _ping_bot(
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start boka init")
        except (InputUserDeactivatedError, ValueError):
            self._db.set("boka.inline", "bot_token", None)
            self._token = False
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start boka init"
                )
            except Exception:
