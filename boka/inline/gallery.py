...
from boka_tl.errors.rpcerrorlist import FloodWaitError, MediaPrevInvalidError
from boka_tl.errors.rpcerrorlist import ChatSendInlineForbiddenError
...
class Gallery(InlineUnit):
    async def gallery(
        ...
        *,
        custom_buttons: BokaReplyMarkup | None = None,
        force_me: bool = False,
        ...
        :param gif: Whether the gallery will be filled with gifs. If you omit this argument and specify
                    gifs in `next_handler`, Boka will try to determine the filetype of these images
        :param manual_security: By default, Boka will try to inherit inline buttons security from the caller (command)
                                If you want to avoid this, pass `manual_security=True`
        :param disable_security: By default, Boka will try to inherit inline buttons security from the caller (command)
        ...
        with contextlib.suppress(AttributeError):
            _boka_client_id_logging_tag = copy.copy(self._client.tg_id)  # noqa: F841
            ...
            try:
                status_message = await (
                    ...
                    (
                        ...
                        if self._client.boka_me.premium
                        else "🪐"
                    )
