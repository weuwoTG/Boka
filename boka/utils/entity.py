...
import boka_tl
import requests
from boka_tl import hints
from boka_tl.tl.custom.message import Message
from boka_tl.tl.functions.account import UpdateNotifySettingsRequest
from boka_tl.tl.functions.channels import CreateChannelRequest, EditPhotoRequest
    ...
    if _folder:
        ...
        try:
            folder = next(
                ...
                if not isinstance(folder, boka_tl.tl.types.DialogFilterDefault)
...
async def asset_forum_topic(
    ...
    async def create_topic() -> ForumTopic:
        result = await client(
            CreateForumTopicRequest(
                peer=entity,
                title=title,
                icon_emoji_id=(icon_emoji_id if client.boka_me.premium else None),
            )
        )
    ...
    forums_cache = db.pointer("boka.forums", "forums_cache", {})
...
async def wait_for_content_channel(db: "Database", delay: float = 10) -> int:
    cid = db.get("boka.forums", "channel_id", None)

    while not cid:
        logger.warning(
            "Boka content channel not found in database. Sleeping 10 seconds..."
        )
...
async def get_topic_id(db: "Database", topic_name: str) -> int | None:
    ...
    """
    try:
        forums_cache = db.get("boka.forums", "forums_cache", {})
        return forums_cache.get("boka-userbot", {}).get(topic_name)
    except Exception:
        return None
...
def get_chat_id(message: Message) -> int:
    ...
    :return: Chat ID
    """
    return boka_tl.utils.resolve_id(
