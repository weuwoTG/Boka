    ...
    try:
        match name:
            ...
            case s if s.startswith("hikka"):
                return native_import("boka" + name[5:], *args, **kwargs)
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?boka$", from_id=659800858)
...
class Modules:
    ...
    def __init__(
        ...
        self.client.boka_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.boka_commands)
                inline_handlers.update(module.boka_inline_handlers)
                callback_handlers.update(module.boka_callback_handlers)
                watchers.extend(module.boka_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.boka_commands))
            )
    ...
    def register_inline_stuff(self, instance: Module):
        ...
        for name, func in instance.boka_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.boka_watchers.values():
