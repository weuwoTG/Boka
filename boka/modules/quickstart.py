...
class Quickstart(loader.Module):
    ...
    async def client_ready(self):
        self.text = lambda: self.strings["base"].format(
            ...
            if self.client.boka_me.premium is True
            else "Boka"
        )

        try:
            content_channel = None
            existing_channel_id = self.db.get("boka.forums", "channel_id", None)
            ...
            if existing_channel_id:
                try:
                    ...
                    content_channel = None
                    self.db.set("boka.forums", "forums_cache", {"boka-userbot": {}})

            if not content_channel:
                async for dialog in self.client.iter_dialogs():
                    if dialog.title and "boka-userbot" in dialog.title.lower():
                        content_channel = dialog.entity
                        logger.debug(
                            f"Found existing channel '{dialog.title}' with ID {dialog.entity.id}"
                        )
                        self.db.set(
                            "boka.forums", "channel_id", int(dialog.entity.id)
                        )
                        break
            ...
            forum_entity = None
            existing_forum_id = self.db.get("boka.forums", "forum_id", None)
            ...
            if not forum_entity:
                try:
                    if not (
                        ...
                        from boka_tl.tl.functions.channels import ToggleForumRequest
                    ...
                    self.db.set("boka.forums", "forum_id", int(content_channel.id))
                except Exception:
                    forum_entity = content_channel
            ...
            required_topics = [
                (
                    "Assets",
                    "🌆 Your Boka assets will be stored here",
                    5877307202888273539,
                ),
                (
                    "Backups",
                    "💾 Your Boka backups will be stored here",
                    5877307202888273539,
                ),
            ]
    ...
    async def lang(self, call: BotInlineCall):
        if not call.data.startswith("boka/lang/"):
            return
