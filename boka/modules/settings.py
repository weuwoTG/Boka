...
class CoreMod(loader.Module):
    ...
    @loader.command(
        ...
        en_doc="Information of Boka",
        ua_doc="Інформація про Хероку",
        de_doc="Informationen über Boka",
    )
    async def bokacmd(self, message: Message):
        ...
        await utils.answer(
            message,
            self.strings["boka"].format(
                (
                    utils.get_platform_emoji()
                    if self._client.boka_me.premium
                    else "🪐 <b>Boka userbot</b>"
                ),
                *version.__version__,
            ...
            + (branch_text),
            file=os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "assets", "boka.png")),
    ...
    async def togglecmdcmd(self, message: Message):
        ...
        if cmd.lower() not in [c.lower() for c in mod_inst.boka_commands.keys()]:
