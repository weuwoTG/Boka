...
from boka_tl.tl.custom import Message
from boka_tl.tl.types import User
from boka_tl.utils import parse_phone
...
@loader.tds
class BokaWebMod(loader.Module):
    """Inline account management"""

    strings = {"name": "BokaAccounts"}
    ...
    def _get_client(self) -> CustomTelegramClient:
        return CustomTelegramClient(
            MemorySession(),
            main.boka.api_token.ID,
            main.boka.api_token.HASH,
            connection=main.boka.conn,
            proxy=main.boka.proxy,
    ...
    def _clone_switch_db(self, old_id: int, new_id: int) -> dict:
        ...
        inline_data = data.setdefault("boka.inline", {})
    ...
    async def _save_switch_session(
        ...
        session = SQLiteSession(os.path.join(main.SESSIONS_DIR, f"boka-{new_id}"))
        session.set_dc(
    ...
    async def _switch_account(self, client: CustomTelegramClient) -> None:
        ...
        new_id = me.id
        if old_id == new_id:
            await main.boka.save_client_session(client, delay_restart=False)
            return

        suffix = int(time.time())
        base = Path(main.SESSIONS_DIR)
        old_session = base / f"boka-{old_id}.session"
        old_journal = base / f"boka-{old_id}.session-journal"
        old_config = Path(main.BASE_PATH) / f"config-{old_id}.json"
        new_session = base / f"boka-{new_id}.session"
        new_journal = base / f"boka-{new_id}.session-journal"
