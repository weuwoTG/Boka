...
from boka_tl.errors.common import ScamDetectionError
from boka_tl.errors.rpcerrorlist import MediaCaptionTooLongError
from boka_tl.tl.functions.channels import JoinChannelRequest
from boka_tl.tl.types import Channel, InputMediaWebPage
...
class LoaderMod(loader.Module):
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?boka_min", doc):
            ver = re.search(r"# ?scope: ?boka_min ((?:\d+\.){2}\d+)", doc).group(1)
            ver_ = tuple(map(int, ver.split(".")))
            if main.__version__ < ver_:
                logger.error(
                    "Module %s requires Boka %s, current version is %s",
                    module_label,
                    ver,
        ...
        module_name = f"boka.modules.{uid}"
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "boka_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.boka_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.boka_entity_cache
                    and getattr(
    ...
    def _is_core_module(self, module) -> bool:
        ...
        if not module_name.startswith("boka.modules."):
            return False
