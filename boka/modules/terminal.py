...
class MessageEditor:
    def __init__(
        self,
        message: boka_tl.tl.types.Message,
        command: str,
        config,
        strings,
        request_message,
    ...
    async def redraw(self):
        ...
        with contextlib.suppress(boka_tl.errors.rpcerrorlist.MessageNotModifiedError):
            try:
...
class SudoMessageEditor(MessageEditor):
    ...
    async def update_stderr(self, stderr):
        ...
        if any(lastlines[0] == i for i in self.PASS_REQ) and self.state == 0:
            ...
            text = self.strings["auth_needed"].format(self.message.client.boka_me.id)

            try:
                await utils.answer(self.message, text)
            except boka_tl.errors.rpcerrorlist.MessageNotModifiedError as e:
                logger.debug(e)
            ...
            self.message.client.add_event_handler(
                ...
                boka_tl.events.messageedited.MessageEdited(chats=["me"]),
            )
...
class RawMessageEditor(SudoMessageEditor):
    ...
    async def redraw(self):
        ...
        with contextlib.suppress(
            boka_tl.errors.rpcerrorlist.MessageNotModifiedError,
            boka_tl.errors.rpcerrorlist.MessageEmptyError,
            ValueError,
        ):
            try:
                await utils.answer(self.message, text)
