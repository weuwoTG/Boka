...
class InlineStuff(loader.Module):
    ...
    @loader.command()
    async def ch_boka_bot(self, message: Message):
        ...
        self._db.set("boka.inline", "custom_bot", args)
    ...
    async def ch_bot_token(self, message: Message):
        ...
        if not args or not re.match(r"[0-9]{8,10}:[a-zA-Z0-9_-]{34,36}", args):
            ...
            return
        self._db.set("boka.inline", "bot_token", args)
        await utils.answer(message, self.strings["bot_updated"])
                ...
                await message.answer_photo(
                    ...
                    caption=self.strings["this_is_boka"].format(
                        (
                            ...
                            if self._client.boka_me.premium is True
                            else "🪐"
                        ),
                        utils.get_platform_emoji() if self._client.boka_me.premium is True else "Boka",
                    ),
                    reply_markup=self.inline.generate_markup(
                        markup_obj=[
                            [
                                {
                                    "text": "GitHub",
                                    "url": "https://github.com/weuwoTG/Boka",
    ...
    async def reset_prefix(self, call: InlineCall, message):
        ...
        self.db.set("boka.main", "command_prefix", ".")
