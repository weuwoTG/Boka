...
@loader.tds
class BokaInfoMod(loader.Module):
    """Show userbot info"""

    strings = {"name": "BokaInfo"}
    ...
    async def _render_info(self, start: float) -> str:
        ...
        me = (
            '<b><a href="tg://user?id={}">{}</a></b>'.format(
                self._client.boka_me.id,
                utils.escape_html(get_display_name(self._client.boka_me)),
            )
            .replace("{", "")
            .replace("}", "")
        )
        ...
        return (
            ...
            if self.config["custom_message"]
                ...
                (
                    ...
                    if self._client.boka_me.premium and self.config["show_boka"]
                    else ""
                ),
                me=me,
                version=_version,
